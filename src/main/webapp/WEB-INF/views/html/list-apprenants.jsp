...
<body>
	<div>
		<nav class="navbar navbar-dark bg-primary">
			<a class="navbar-brand"
				...
				height="40px" width="180px" alt=""> </a> <br> <br>



			<form class="form-inline my-2 my-lg-0" method="post"
				action="${pageContext.request.contextPath}/deconnect">
	...
	<div class="container mt-5 pt-3">
		...
		<form action="demandesAcces" method="post">
			<div class="input-group mb-3">
				<span class="input-group-text">sélectionner la date</span> <select
					class="form-select form-control" required name="id_date">
					<option value="">sélectionner la date</option>
					<c:forEach items="${ ListAgenda }" var="reserver">
						<option value="${ reserver.id_agenda }">
							${reserver.desc_agenda}</option>
					</c:forEach>
				</select> <select class="form-select form-control" required
					name="id_fabrique">
					<option value="">sélectionner la fabrique</option>
					<c:forEach items="${ ListFabrique }" var="reserverF">
						<option value="${ reserverF.id_fabrique }">
							${reserverF.nom_fabrique}</option>
					</c:forEach>
				</select>
	...
	<div>
		<table class="table table-dark">
			...
			<tbody>
				<c:forEach items="${ListResByUser}" var="l">
					...
					<tr>
						...
						<td><c:if test="${l.active_reservation}">
                              Accepter
                             </c:if>
                              <c:if test="${!l.active_reservation}">
                              Refuser
                              </c:if></td>
					</tr>
				</c:forEach>
